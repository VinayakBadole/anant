...
class ThemeTransition {
  ...
  setupScrollListener() {
    // Prefer Lenis-driven custom event when available to avoid duplicate RAF/tickers
    this.handleSmoothScroll = (e) => {
      if (!e || !e.detail) return
      this.scrollProgress = typeof e.detail.progress === 'number' ? e.detail.progress : this.scrollProgress
    ...
    window.addEventListener('smoothScroll', this.handleSmoothScroll, { passive: true })

    // Fallback to native scroll for when Lenis is disabled
    this._nativeScrollHandler = () => {
  ...
  destroy() {
    ...
    if (this.handleSmoothScroll) {
      window.removeEventListener('smoothScroll', this.handleSmoothScroll)
      this.handleSmoothScroll = null
    }
    if (this._nativeScrollHandler) {
      window.removeEventListener('scroll', this._nativeScrollHandler)
      this._nativeScrollHandler = null
    }
